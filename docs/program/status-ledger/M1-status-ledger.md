...
| TASK-M1-009 | Model/Profile Discovery Records | TASK-M1-001 | PG-M1-02A | IMPLEMENTED, TESTED | Local model/profile discovery records are implemented in the provider/profile boundary with deterministic fakeable tests, no-secret metadata, unavailable-provider behavior, and no model generation. |
